...
substrates <- read_tsv('./data/kinase_data/serine_threonine_kinases/computing_aprior_distributions/substrates.tsv')
kinase_data <- fread('./data/kinase_data/serine_threonine_kinases/kinase_motifs.csv')
md <- tibble(
...
dir.create('./data/kinase_data/serine_threonine_kinases/apriori_distributions/')

for (i in 1:nrow(kinase_data)){
  elements <- sapply(log2_scores, function(x) x[[i]])
  write_delim(data.frame(elements), paste0('./data/kinase_data/serine_threonine_kinases/apriori_distributions/', kinase_data$KINASE[i], '.txt'), col_names = FALSE)
}
